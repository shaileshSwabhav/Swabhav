...
//		checkCases(c);
	...
	public static void printNumberOfAnnotation(CustomAnnotation c) throws Exception {
...
//					System.out.println(m.getName());
	...
	public static void checkCases(CustomAnnotation c) throws Exception {
		 ...
		 * for (Method m : method) {
		 * 
		 * Annotation[] annot = m.getDeclaredAnnotations(); for (Annotation a : annot) {
		 * 
		 * m.setAccessible(true); boolean value = (boolean) m.invoke(c, null); if (value
		 * == isTrue) { countPassedCases++; // System.out.println(m.getName()); } else {
		 * countFailedCases++; } } }
